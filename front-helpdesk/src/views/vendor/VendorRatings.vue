<template>
  <div class="container-xxl flex-grow-1 container-p-y vendor-ratings-page">
    ...
    <div v-if="stats.warnings?.length" class="panel-card">
      ...
      <div class="ticket-rating-list">
        <div
          v-for="warning in stats.warnings"
          :key="warning.id"
          :class="['ticket-rating-item', { 'ticket-rating-item--alert': warning.warning_type === 'admin' }]"
        >
          <div class="ticket-rating-top">
            <div class="ticket-rating-copy">
              <div class="ticket-number">{{ warning.warning_type === 'admin' ? 'Peringatan Admin' : 'Peringatan Sistem' }}</div>
              <h6>{{ warning.message }}</h6>
              <p>{{ formatWarningDate(warning.created_at) }}</p>
            </div>

            <div class="ticket-rating-status">
              <span :class="['rating-status-badge', warning.warning_type === 'admin' ? 'rated' : 'pending']">
    ...
    <div class="stats-grid">
      ...
      <div class="stat-card">
        ...
        <strong class="stat-value">{{ stats.system_warning_count || 0 }}</strong>
      </div>
      <div class="stat-card stat-card--warning">
        <span class="stat-label">Warning Admin</span>
        <strong class="stat-value">{{ stats.admin_warning_count || 0 }}</strong>
      </div>
    </div>
...
const formatWarningDate = (date) => new Date(date).toLocaleString('id-ID', {
  day: '2-digit',
  month: 'long',
  year: 'numeric',
  hour: '2-digit',
